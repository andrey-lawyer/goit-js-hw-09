...
function handleSubmit(event) {
    ...
    const { elements: { delay: delay1 , step , amount  } ,  } = event.currentTarget;
    console.log(delay1.value, step.value, amount.value);
    const dalayNumber = Number(delay1.value);
    const stepNumber = Number(step.value);
    const amountNumber = Number(amount.value);
    let dalayAdd = dalayNumber;
    for(let i = 0; i < amountNumber; i += 1){
        createPromise(i + 1, dalayAdd).then(({ position , delay  })=>{
